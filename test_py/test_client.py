...
def describe_client():
    ...
    def describe_download_report():
        ...
        def it_downloads_report_as_package(client, download_file, api_mock):
            ...
            assert download_file.call_count == 3

            (args, _) = download_file.call_args_list[0]

            assert args == (
                'report url',
                join_path(getcwd(), 'TEST_REPORT', 'TEST_KEY_1.pdf'),
            )

            (args, _) = download_file.call_args_list[1]

            assert args == (
                'export url',
                join_path(getcwd(), 'TEST_REPORT', 'TEST_KEY_2.csv'),
            )

            (args, _) = download_file.call_args_list[2]

            assert args == (
                'file url',
                join_path(getcwd(), 'TEST_REPORT', 'TEST_KEY_3.zip'),
            )

        def it_downloads_report_file_to_filename(client, download_file, api_mock):
            api_mock.get(
                url=f'{FILES_URL}/TEST_KEY_2.zip',
                status=200,
                json={'url': 'file url'},
            )
            ...
            assert args == (
                'file url',
                join_path(getcwd(), 'TEST_REPORT', 'TEST_FILE.zip'),
            )
