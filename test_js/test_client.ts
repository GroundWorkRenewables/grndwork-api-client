...
describe('Client', () => {
  ...
  describe('downloadReport', () => {
    ...
    it('downloads report as package', async () => {
      ...
      expect(downloadFile).toHaveBeenCalledWith(
        'report url',
        joinPath(getcwd(), 'TEST_REPORT', 'TEST_KEY_1.pdf'),
        {timeout: undefined},
      );

      expect(downloadFile).toHaveBeenCalledWith(
        'export url',
        joinPath(getcwd(), 'TEST_REPORT', 'TEST_KEY_2.csv'),
        {timeout: undefined},
      );

      expect(downloadFile).toHaveBeenCalledWith(
        'file url',
        joinPath(getcwd(), 'TEST_REPORT', 'TEST_KEY_3.zip'),
        {timeout: undefined},
      );
    });

    it('downloads report file to filename', async () => {
      apiMock.intercept({
        path: `${FILES_PATH}/TEST_KEY_2.zip`,
      })
      .reply(200, {url: 'file url'});
      ...
      expect(downloadFile).toHaveBeenCalledWith(
        'file url',
        joinPath(getcwd(), 'TEST_REPORT', 'TEST_FILE.zip'),
        {timeout: undefined},
      );
    });
